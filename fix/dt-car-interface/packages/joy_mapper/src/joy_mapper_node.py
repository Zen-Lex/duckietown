...
class JoyMapperNode(DTROS):
    ...
    **Joystick bindings:**
	...
	# Joystick Control disabled by default
        ...
        self.pub_rec_mode = rospy.Publisher(
            "~recovery_mode", BoolStamped, queue_size=1, dt_topic_type=TopicType.CONTROL
        )
    ...
    def joy_cb(self, joy_msg):
        ...
        car_cmd_msg.v = joy_msg.axes[1] * self._speed_gain * self.speed_modifier[self.fsm_mode]
        if self._bicycle_kinematics:
            ...
            car_cmd_msg.omega = car_cmd_msg.v / self._simulated_vehicle_length * math.tan(steering_angle) * self.speed_modifier[self.fsm_mode]
        else:
            # Holonomic Kinematics for Normal Driving
            car_cmd_msg.omega = joy_msg.axes[3] * self._steer_gain * self.speed_modifier[self.fsm_mode]
